...
`OpenDirect.Order`/`Line`/`ChangeRequest` exist, with `preferredbillingmethod`, `leadtime` and `reservedexpirydate` (soft holds); AdCP `commercial-terms` adds `invoice_recipient`. The surviving requirement moves to ADD-013.
